...
#include <inery/action.h>
#include <inery/chain.h>
#include <inery/crypto.h>
#include <inery/db.h>
#include <inery/permission.h>
#include <inery/print.h>
#include <inery/privileged.h>
#include <inery/system.h>
#include <inery/transaction.h>
#include <inery/types.h>

#include <type_traits>

namespace inery { namespace native {
   ...
   auto create_function(std::index_sequence<Is...>) {
      return std::function<R(typename std::tuple_element<Is, Args>::type ...)>{
         [](typename std::tuple_element<Is, Args>::type ...) {
            inery_assert(false, "unsupported intrinsic"); return (R)0;
         }
      };
   }
...
intrinsic_macro(set_proposed_masters) \
intrinsic_macro(set_proposed_masters_ex) \
...
intrinsic_macro(get_active_masters) \
...
#define GENERATE_TYPE_MAPPING(name) \
   struct __ ## name ## _types { \
      using deduced_full_ts = decltype(inery::native::get_args_full(::name)); \
      using deduced_ts      = decltype(inery::native::get_args(::name)); \
...
#define GET_TYPE(name) \
   decltype(create_function<inery::native::intrinsics::__ ## name ## _types::res_t, \
         inery::native::intrinsics::__ ## name ## _types::deduced_full_ts>(inery::native::intrinsics::__ ## name ## _types::is)),

#define REGISTER_INTRINSIC(name) \
   create_function<inery::native::intrinsics::__ ## name ## _types::res_t, \
         inery::native::intrinsics::__ ## name ## _types::deduced_full_ts>(inery::native::intrinsics::__ ## name ## _types::is),
