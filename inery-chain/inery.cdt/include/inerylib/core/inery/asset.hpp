...
namespace inery {
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         inery::check( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         inery::check( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         inery::check( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         inery::check( -max_amount <= amount, "subtraction underflow" );
         inery::check( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }
       ...
       */
      asset& operator+=( const asset& a ) {
         inery::check( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         inery::check( -max_amount <= amount, "addition underflow" );
         inery::check( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         ...
         inery::check( tmp <= max_amount, "multiplication overflow" );
         inery::check( tmp >= -max_amount, "multiplication underflow" );
         amount = (int64_t)tmp;
         return *this;
      }
       ...
       */
      asset& operator/=( int64_t a ) {
         inery::check( a != 0, "divide by zero" );
         inery::check( !(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow" );
         amount /= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         inery::check( b.amount != 0, "divide by zero" );
         inery::check( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
      ...
      INELIB_SERIALIZE( asset, (amount)(symbol) )
   };
   ...
   */
   struct extended_asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         inery::check( a.contract == b.contract, "type mismatch" );
      ...
      INELIB_SERIALIZE( extended_asset, (quantity)(contract) )
   };
}
