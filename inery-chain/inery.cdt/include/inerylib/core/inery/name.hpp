...
namespace inery {
   ...
   public:
      ...
      {
         if( str.size() > 13 ) {
            inery::check( false, "string is too long to be a valid name" );
         }
         if( str.empty() ) {
            return;
         }
         ...
         if( str.size() == 13 ) {
            ...
            if( v > 0x0Full ) {
               inery::check(false, "thirteenth character in name cannot be a letter that comes after j");
            }
            value |= v;
         }
      }
      ...
      static constexpr uint8_t char_to_value( char c ) {
         if( c == '.')
            ...
            inery::check( false, "character is not in allowed character set for names" );
...
inline constexpr inery::name operator""_n() {
   constexpr auto x = inery::name{std::string_view{inery::detail::to_const_char_arr<Str...>::value, sizeof...(Str)}};
   return x;
}
