...
#define INELIB_REFLECT_MEMBER_OP( r, OP, elem ) \
  OP t.elem
 ...
 */
#define INELIB_SERIALIZE( TYPE,  MEMBERS ) \
 template<typename DataStream> \
 friend DataStream& operator << ( DataStream& ds, const TYPE& t ){ \
    return ds BOOST_PP_SEQ_FOR_EACH( INELIB_REFLECT_MEMBER_OP, <<, MEMBERS );\
 }\
 template<typename DataStream> \
 ...
 *  @param MEMBERS - a sequence of member names.  (field1)(field2)(field3)
 */
#define INELIB_SERIALIZE_DERIVED( TYPE, BASE, MEMBERS ) \
 template<typename DataStream> \
 friend DataStream& operator << ( DataStream& ds, const TYPE& t ){ \
