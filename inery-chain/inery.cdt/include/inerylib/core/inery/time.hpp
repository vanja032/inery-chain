...
namespace inery {
  ...
  class microseconds {
    public:
        ...
        int64_t _count;
        /// @endcond
        INELIB_SERIALIZE( microseconds, (_count) )
    private:
  ...
  class time_point {
    public:
        ...
        INELIB_SERIALIZE( time_point, (elapsed) )
  };
  ...
  class time_point_sec
  {
    public:
        ...
        operator time_point()const { return time_point( inery::seconds( utc_seconds) ); }
        uint32_t sec_since_epoch()const { return utc_seconds; }

        /// @cond INTERNAL
        time_point_sec operator = ( const inery::time_point& t )
        {
          utc_seconds = uint32_t(t.time_since_epoch().count() / 1000000ll);
          return *this;
        }
        ...
        INELIB_SERIALIZE( time_point_sec, (utc_seconds) )
  };
   ...
   class block_timestamp {
      public:
         ...
         block_timestamp next() const {
            inery::check( std::numeric_limits<uint32_t>::max() - slot >= 1, "block timestamp overflow" );
         ...
         INELIB_SERIALIZE( block_timestamp, (slot) )
      private:
