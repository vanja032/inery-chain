   ...
   public:
      ...
      */
      inline bool read( char* d, size_t s ) {
        inery::check( size_t(_end - _pos) >= (size_t)s, "read" );
        memcpy( d, _pos, s );
        _pos += s;
        return true;
      }
      ...
      */
      inline bool write( const char* d, size_t s ) {
        inery::check( _end - _pos >= (int32_t)s, "write" );
...
void deserialize(datastream<Stream>& ds, std::variant<Ts...>& var, int i) {
   if constexpr (I < std::variant_size_v<std::variant<Ts...>>) {
      ...
      inery::check(false, "invalid variant index");
   }
}
...
DataStream& operator >> ( DataStream& ds, T (&v)[N] ) {
   unsigned_int s;
   ds >> s;
   inery::check( N == s.value, "T[] size and unpacked size don't match");
   for( uint32_t i = 0; i < N; ++i )
      ds >> v[i];
   return ds;
}
