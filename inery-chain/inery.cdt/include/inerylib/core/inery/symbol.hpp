...
namespace inery {
   ...
   */
   ...
   public:
      ...
      {
         if( str.size() > 7 ) {
            inery::check( false, "string is too long to be a valid symbol_code" );
         }
         for( auto itr = str.rbegin(); itr != str.rend(); ++itr ) {
            if( *itr < 'A' || *itr > 'Z') {
               inery::check( false, "only uppercase letters allowed in symbol_code string" );
            }
            value <<= 8;
            value |= *itr;
         }
   ...
   inline DataStream& operator<<(DataStream& ds, const inery::symbol_code sym_code) {
    ...
    */
   template<typename DataStream>
   inline DataStream& operator>>(DataStream& ds, inery::symbol_code& sym_code) {
     uint64_t raw = 0;
   ...
   public:
      ...
      void print( bool show_precision = true )const {
         if( show_precision ){
            ::inery::print( static_cast<uint64_t>(precision()), "," );
         }
         char buffer[7];
    ...
    */
   template<typename DataStream>
   inline DataStream& operator<<(DataStream& ds, const inery::symbol sym) {
    ...
    */
   template<typename DataStream>
   inline DataStream& operator>>(DataStream& ds, inery::symbol& sym) {
     uint64_t raw = 0;
     ds.read((char*)&raw, sizeof(raw));
     sym = symbol(raw);
     return ds;
   ...
   private:
      ...
      INELIB_SERIALIZE( extended_symbol, (sym)(contract) )
   };
}
