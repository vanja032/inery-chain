...
#include "../../core/inery/datastream.hpp"
#include "../../core/inery/name.hpp"
#include "../../core/inery/ignore.hpp"
...
namespace inery {
    ...
    *    int  c; //4
    *
    *    INELIB_SERIALIZE( dummy_action, (a)(b)(c) )
    *  };
   ...
   */
   ...
   struct permission_level {
      ...
      INELIB_SERIALIZE( permission_level, (actor)(permission) )
   };
   ...
   struct action {
      ...
      INELIB_SERIALIZE( action, (account)(name)(authorization)(data) )
      ...
      void send_context_free() const {
         inery::check( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
    ...
    * transfer_act{"inery.token"_n, {st.issuer, "active"_n}}.send(st.issuer, to, quantity, memo);
    * // or
    * transfer_act trans_action{ "inery.token"_n, {st.issuer, "active"_n}};
    * trans_action.send(st.issuer, to, quantity, memo);
    * @endcode
    */
   template <inery::name::raw Name, auto Action>
   struct action_wrapper {
      template <typename Code>
      constexpr action_wrapper(Code&& code, std::vector<inery::permission_level>&& perms)
         : code_name(std::forward<Code>(code)), permissions(std::move(perms)) {}

      template <typename Code>
      constexpr action_wrapper(Code&& code, const std::vector<inery::permission_level>& perms)
         : code_name(std::forward<Code>(code)), permissions(perms) {}

      template <typename Code>
      constexpr action_wrapper(Code&& code, inery::permission_level&& perm)
         : code_name(std::forward<Code>(code)), permissions({1, std::move(perm)}) {}

      template <typename Code>
      constexpr action_wrapper(Code&& code, const inery::permission_level& perm)
      ...
      static constexpr inery::name action_name = inery::name(Name);
      inery::name code_name;
      std::vector<inery::permission_level> permissions;
   ...
   template <inery::name::raw Name, auto... Actions>
   struct variant_action_wrapper {
      template <typename Code>
      constexpr variant_action_wrapper(Code&& code, std::vector<inery::permission_level>&& perms)
         : code_name(std::forward<Code>(code)), permissions(std::move(perms)) {}

      template <typename Code>
      constexpr variant_action_wrapper(Code&& code, const std::vector<inery::permission_level>& perms)
         : code_name(std::forward<Code>(code)), permissions(perms) {}

      template <typename Code>
      constexpr variant_action_wrapper(Code&& code, inery::permission_level&& perm)
         : code_name(std::forward<Code>(code)), permissions({1, std::move(perm)}) {}

      template <typename Code>
      constexpr variant_action_wrapper(Code&& code, const inery::permission_level& perm)
         : code_name(std::forward<Code>(code)), permissions({1, perm}) {}
...
::inery::inline_dispatcher<decltype(&CONTRACT_CLASS::FUNCTION_NAME), ACTION_NAME>::call

#define INLINE_ACTION_SENDER2( CONTRACT_CLASS, NAME )\
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::inery::name(#NAME) )
 ...
 * INERY supports two communication models, inline and deferred. Inline actions are executed as part of the current transaction. This macro
 ...
 * @endcode
 * 
 * The example above is taken from inery.token. 
 * This example:  
 *       uses the passed in, dereferenced `this` pointer, to call this.get_self() i.e. the inery.token contract; 
 *       calls the inery.token::transfer() action; 
