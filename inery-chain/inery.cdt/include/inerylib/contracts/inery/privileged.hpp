...
namespace inery {
   ...
   namespace internal_use_do_not_use {
      extern "C" {
         ...
         __attribute((inery_wasm_import))
         int64_t set_proposed_masters( char*, uint32_t );

         __attribute__((inery_wasm_import))
         void preactivate_feature( const capi_checksum256* feature_digest );

         __attribute__((inery_wasm_import))
         int64_t set_proposed_masters_ex( uint64_t producer_data_format, char *producer_data, uint32_t producer_data_size );
      }
   }
   ...
   */
   ...
   struct blockchain_parameters {
      ...
      */
      ...
      INELIB_SERIALIZE( blockchain_parameters,
    ...
    */
   void set_blockchain_parameters(const inery::blockchain_parameters& params);
    ...
    */
   void get_blockchain_parameters(inery::blockchain_parameters& params);
    ...
    */
   std::optional<uint64_t> set_proposed_masters( const std::vector<master_key>& prods );
    ...
    *  @return an optional value of the version of the new proposed schedule if successful
    */
   inline std::optional<uint64_t> set_proposed_masters( const std::vector<master_authority>& prods ) {
      auto packed_prods = inery::pack( prods );
      int64_t ret = internal_use_do_not_use::set_proposed_masters_ex(1, (char*)packed_prods.data(), packed_prods.size());
      if (ret >= 0)
