...
#include "../../contracts/inery/action.hpp"
#include "../../core/inery/name.hpp"
#include "../../core/inery/serialize.hpp"
...
namespace _multi_index_detail {
   ...
   WRAP_SECONDARY_ARRAY_TYPE(idx256, inery::fixed_bytes<32>)
   template<>
   struct secondary_key_traits<inery::fixed_bytes<32>> {
      static constexpr inery::fixed_bytes<32> true_lowest() { return inery::fixed_bytes<32>(); }
   };

}

/**
 *  The indexed_by struct is used to instantiate the indices for the Multi-Index table. In INERY, up to 16 secondary indices can be specified.
 *
 *  @ingroup multiindex
 *  @tparam IndexName - is the name of the index. The name must be provided as an INERY base32 encoded 64-bit integer and must conform to the INERY naming requirements of a maximum of 13 characters, the first twelve from the lowercase characters a-z, digits 1-5, and ".", and if there is a 13th character, it is restricted to lowercase characters a-p and ".".
 *  @tparam Extractor - is a function call operator that takes a const reference to the table object type and returns either a secondary key type or a reference to a secondary key type. It is recommended to use the `inery::const_mem_fun` template, which is a type alias to the `boost::multi_index::const_mem_fun`. See the documentation for the Boost `const_mem_fun` key extractor for more details.
 *
 *  Example:
       *
*
 ...
 *  class mycontract: inery::contract {
 *    struct record {
 *       uint64_t    primary;
 *       uint128_t   secondary;
 ...
 *      }
 *  }
 *  INERY_DISPATCH( mycontract, (myaction) )
 *  @endcode
 */
 ...
 *  @ingroup contracts
 *
 *  @brief Defines INERY Multi Index Table
 *  @details INERY Multi-Index API provides a C++ interface to the INERY database. It is patterned after Boost Multi Index Container.
 *  INERY Multi-Index table requires exactly a uint64_t primary key. For the table to be able to retrieve the primary key,
 *  the object stored inside the table is required to have a const member function called primary_key() that returns uint64_t.
 *  INERY Multi-Index table also supports up to 16 secondary indices. The type of the secondary indices could be any of:
 *  - uint64_t
 *  - uint128_t
 *  - double
 *  - long double
...
class multi_index
{
   private:
      ...
      struct index {
         public:
            ...
            constexpr static bool validate_index_name( inery::name n ) {
               return n.value != 0 && n != inery::name("primary"); // Primary is a reserve index name.
            }
            ...
            struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
               public:
                  ...
                  const_iterator& operator++() {
                     ...
                     inery::check( _item != nullptr, "cannot increment end iterator" );
                  ...
                  const_iterator& operator--() {
                     ...
                     if( !_item ) {
                        ...
                        inery::check( ei != -1, "cannot decrement end iterator when the index is empty" );
                        prev_itr = secondary_index_db_functions<secondary_key_type>::db_idx_previous( ei , &prev_pk );
                        inery::check( prev_itr >= 0, "cannot decrement end iterator when the index is empty" );
                     } else {
                        if( _item->__iters[Number] == -1 ) {
                           secondary_key_type temp_secondary_key;
                           auto idxitr = secondary_index_db_functions<secondary_key_type>::db_idx_find_primary(_idx->get_code().value, _idx->get_scope(), _idx->name(), _item->primary_key(), temp_secondary_key);
                        ...
                        inery::check( prev_itr >= 0, "cannot decrement iterator at beginning of index" );
                     }
            ...
            const_iterator require_find( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               auto lb = lower_bound( secondary );
               inery::check( lb != cend(), error_msg );
               inery::check( secondary == secondary_extractor_type()(*lb), error_msg );
               return lb;
            }
            ...
            const T& get( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               ...
               inery::check( result != cend(), error_msg );
               return *result;
            }
            ...
            const_iterator iterator_to( const T& obj ) {
               ...
               inery::check( objitem.__idx == _multidx, "object passed to iterator_to is not in multi_index" );
            ...
            void modify( const_iterator itr, inery::name payer, Lambda&& updater ) {
               inery::check( itr != cend(), "cannot pass end iterator to modify" );

               _multidx->modify( *itr, payer, std::forward<Lambda&&>(updater) );
            }

            const_iterator erase( const_iterator itr ) {
               inery::check( itr != cend(), "cannot pass end iterator to erase" );
            ...
            inery::name get_code()const  { return _multidx->get_code(); }
      ...
      static constexpr auto transform_indices( ) {
         ...
         return hana::transform( indices_input_type(), [&]( auto&& idx ){
             ...
             return hana::make_tuple( hana::type_c<index<inery::name::raw(static_cast<uint64_t>(idx_type::index_name)),
                                                         typename idx_type::secondary_extractor_type,
                                                         num_type::e::value, false> >,
                                      hana::type_c<index<inery::name::raw(static_cast<uint64_t>(idx_type::index_name)),
                                                         typename idx_type::secondary_extractor_type,
      ...
      const item& load_object_by_primary_iterator( int32_t itr )const {
         ...
         inery::check( size >= 0, "error reading iterator" );
   ...
   public:
       ...
       *
       *  Notes
       *  The `inery::multi_index` template has template parameters `<name::raw TableName, typename T, typename... Indices>`, where:
       *  - `TableName` is the name of the table, maximum 12 characters long, characters in the name from the set of lowercase letters, digits 1 to 5, and the "." (period) character and is converted to a inery::raw - which wraps uint64_t;
       ...
       *  - It is recommended to use the inery::const_mem_fun template, which is a type alias to the boost::multi_index::const_mem_fun.  See the documentation for the Boost const_mem_fun key extractor for more details.
       *
       *  Example:
       *
       *  @code
       ...
       *      typedef inery::multi_index< "address"_n, address > address_index;
       *      void myaction() {
       *        address_index addresses(_self, _self.value); // code, scope
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
       */
       ...
       *        inery::check(addresses.get_code() == "dan"_n, "Codes don't match.");
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *      void myaction() {
       *        address_index addresses("dan"_n, "dan"_n); // code, scope
       *        inery::check(addresses.get_code() == "dan"_n, "Scopes don't match");
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
         ...
         const_iterator& operator--() {
            ...
            if( !_item ) {
               ...
               inery::check( ei != -1, "cannot decrement end iterator when the table is empty" );
               prev_itr = internal_use_do_not_use::db_previous_i64( ei , &prev_pk );
               inery::check( prev_itr >= 0, "cannot decrement end iterator when the table is empty" );
            } else {
               prev_itr = internal_use_do_not_use::db_previous_i64( _item->__primary_itr, &prev_pk );
               inery::check( prev_itr >= 0, "cannot decrement iterator at beginning of table" );
            }
       ...
       *        inery::check(itr == addresses.cbegin(), "Only address is not at front.");
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        inery::check(itr == addresses.begin(), "Only address is not at front.");
       *      }
       *  }
       *  INERY_ABI( addressbook, (myaction) )
       *  @endcode
       */
      const_iterator begin()const  { return cbegin(); }
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        inery::check(itr != addresses.cend(), "Address for account doesn't exist");
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        inery::check(itr != addresses.end(), "Address for account doesn't exist");
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *          address.street = "1 INE Way";
       *          address.city = "Hong Kong";
       *          address.state = "HK";
       *        });
       *        auto itr = addresses.crbegin();
       *        inery::check(itr->account_name == name("dan"), "Lock arf, Incorrect Last Record ");
       *        itr++;
       *        inery::check(itr->account_name == name("brendan"), "Lock arf, Incorrect Second Last Record");
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        itr--;
       *        inery::check(itr->account_name == name("brendan"), "Lock arf, Incorrect First Record ");
       *        itr--;
       *        inery::check(itr->account_name == name("dan"), "Lock arf, Incorrect Second Record");
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        inery::check(itr->account_name == name("brendan"), "Lock arf, Incorrect First Lower Bound Record ");
       *        itr++;
       *        inery::check(itr->account_name == name("dan"), "Lock arf, Incorrect Second Lower Bound Record");
       *        itr++;
       *        inery::check(itr == zip_index.end(), "Lock arf, Incorrect End of Iterator");
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        inery::check(itr->account_name == name("dan"), "Lock arf, Incorrect First Upper Bound Record ");
       *        itr++;
       *        inery::check(itr == zip_index.end(), "Lock arf, Incorrect End of Iterator");
       *      }
       *  }
      ...
      uint64_t available_primary_key()const {
         ...
         inery::check( _next_primary_key < no_available_primary_key, "next primary key in table is at autoincrement limit");
       ...
       *      typedef inery::multi_index< name("address"), address, indexed_by< name("zip"), const_mem_fun<address, uint64_t, &address::by_zip> > address_index;
       *      void myaction() {
       *        // create reference to address_index  - see emplace example below
       *        // add dan account to table           - see emplace example below
       ...
       *        inery::check(itr->account_name == name("dan"), "Lock arf, Incorrect Record ");
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        inery::check(iterator_to(user) == itr, "Invalid iterator");
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      const_iterator iterator_to( const T& obj )const {
         const auto& objitem = static_cast<const item&>(obj);
         inery::check( objitem.__idx == this, "object passed to iterator_to is not in multi_index" );
         return {this, &objitem};
      }
      /**
      ...
      const_iterator emplace( name payer, Lambda&& constructor ) {
         ...
         inery::check( _code == current_receiver(), "cannot create objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        inery::check(itr != addresses.end(), "Address for account not found");
      ...
      void modify( const_iterator itr, name payer, Lambda&& updater ) {
         inery::check( itr != end(), "cannot pass end iterator to modify" );
       ...
       *          address.state = "CA";
       *        });
       *        inery::check(itr->city == "San Luis Obispo", "Lock arf, Address not modified");
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      void modify( const T& obj, name payer, Lambda&& updater ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         inery::check( objitem.__idx == this, "object passed to modify is not in multi_index" );
         auto& mutableitem = const_cast<item&>(objitem);
         inery::check( _code == current_receiver(), "cannot modify objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         inery::check( pk == obj.primary_key(), "updater cannot change primary key when modifying an object" );
       ...
       *
       *        auto user = addresses.get("dan"_n);
       *        inery::check(user.first_name == "Daniel", "Couldn't get him.");
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        inery::check(itr != addresses.end(), "Couldn't get him.");
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        inery::check(itr != addresses.end(), "Address for account not found");
       *        addresses.erase( itr );
       *        inery::check(itr != addresses.end(), "Everting lock arf, Address not erased properly");
       *      }
       *  }
       *  INERY_ABI( addressbook, (myaction) )
       *  @endcode
       */
      const_iterator erase( const_iterator itr ) {
         inery::check( itr != end(), "cannot pass end iterator to erase" );
       ...
       *      void myaction() {
       *        auto itr = addresses.find("dan"_n);
       *        inery::check(itr != addresses.end(), "Record is not found");
       *        addresses.erase(*itr);
       *        itr = addresses.find("dan"_n);
       *        inery::check(itr == addresses.end(), "Record is not deleted");
       *      }
       *  }
       *  INERY_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      void erase( const T& obj ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         inery::check( objitem.__idx == this, "object passed to erase is not in multi_index" );
         inery::check( _code == current_receiver(), "cannot erase objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         inery::check( itr2 != _items_vector.rend(), "attempt to remove object that was not in multi_index" );
