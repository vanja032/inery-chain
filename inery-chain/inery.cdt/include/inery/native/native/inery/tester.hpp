...
inline bool expect_assert(bool check, const std::string& li, Pred&& pred, F&& func, Args... args) {
   ...
   if (ret == 0) {
      ...
      if (!check)
         inery::check(false, std::string("error : expect_assert, no assert {"+li+"}").c_str());
      inery::print("error : expect_assert, no assert {"+li+"}\n");
   ...
   silence_output(false);
   if (!check)
      inery::check(passed, std::string("error : expect_assert, wrong assert {"+li+"}").c_str());
   if (!passed)
      inery::print("error : expect_assert, wrong assert {"+li+"}\n");
   silence_output(disable_out);
...
inline bool expect_print(bool check, const std::string& li, Pred&& pred, F&& func, Args... args) {
   ...
   silence_output(false);
   if (!check)
      inery::check(passed, std::string("error : wrong print message {"+li+"}").c_str());
   if (!passed)
      inery::print("error : wrong print message {"+li+"}\n");
   silence_output(disable_out);
   return passed;
}
...
#define CHECK_EQUAL(X, Y) \
   if (!(X == Y)) { \
      ___has_failed = true; \
      inery::print(std::string("CHECK_EQUAL failed (")+#X+" != "+#Y+") {"+__FILE__+":"+std::to_string(__LINE__)+"}\n"); \
   }

#define REQUIRE_EQUAL(X, Y) \
   inery::check(X == Y, std::string(std::string("REQUIRE_EQUAL failed (")+#X+" != "+#Y+") {"+__FILE__+":"+std::to_string(__LINE__)+"}").c_str());
...
#define INERY_TEST(X) \
   ...
   if ( X ## _ret == 0 ) \
      ...
      silence_output(false); \
      inery::print("\033[1;37m", #X, " \033[0;37munit test \033[1;31mfailed\033[0m (aborted)\n"); \
      ___has_failed = true; \
      silence_output(___original_disable_output); \
   }

#define INERY_TEST_BEGIN(X) \
   void X() { \
      static constexpr const char* __test_name = #X; \
      ___earlier_unit_test_has_failed = ___has_failed; \
      ___has_failed = false;

#define INERY_TEST_END \
      bool ___original_disable_output = ___disable_output; \
      silence_output(false); \
      if (___has_failed) \
         inery::print("\033[1;37m", __test_name, " \033[0;37munit test \033[1;31mfailed\033[0m\n"); \
      else \
         inery::print("\033[1;37m", __test_name, " \033[0;37munit test \033[1;32mpassed\033[0m\n"); \
      silence_output(___original_disable_output); \
